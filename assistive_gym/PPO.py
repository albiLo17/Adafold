    ...
    def act(self, state, past_actions=None):
        if self.use_pointnet:
            if past_actions is None:
                past_actions = torch.zeros(self.action_dim).to(state.device)
            state = self.encoder(state.view(-1,3).unsqueeze(0).unsqueeze(0), past_actions.unsqueeze(0).unsqueeze(0))
