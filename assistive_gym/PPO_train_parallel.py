...
def train():
    ...
    action_std_decay_freq = 3000 #int(2.5e5)  # action_std decay frequency (in num timesteps)
    #####################################################
    ...
    num_env = 4
    # TODO: randomization of the environments needs to happen here or inside the env.reset()
    # env = normalize(HalfFoldEnv(frame_skip=10, hz=50, action_mult=0.02))
    env = HalfFoldEnv(frame_skip=10, hz=50, action_mult=0.02)
    vec_env = gym.vector.AsyncVectorEnv(
        [lambda: env for i in range(num_env)]
    )
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len + 1):
            ...
            ppo_agent.buffer.load(rewards=reward.reshape(reward.shape[0], -1), is_terminals=done.reshape(done.shape[0], -1))
        ...
        if writer is not None:
            ...
            writer.add_scalar("Env/final_reward", reward.mean(), i_episode + 1)
