...
class DressingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        # action = np.ones(7)
        if self.use_ik:
            self.take_step(action, action_multiplier=1, ik=True)#0.01, ik=True)
        else:
        ...
        if self.forearm_in_sleeve:
            if self.upperarm_in_sleeve:
                print('Both arms inserted')
            else:
                print('forearm inserted')
    ...
    def _get_obs(self, agent=None):
        #TODO: add cloth mesh as observation and maybe force measurements and make sure that the cloth is in the right reference frame
        ...
        # TODO: check cloth forces
        x, y, z, cx, cy, cz, fx, fy, fz = p.getSoftBodyData(self.cloth, physicsClientId=self.id)
        mesh_points = np.concatenate(
            [np.expand_dims(x, axis=-1), np.expand_dims(y, axis=-1), np.expand_dims(z, axis=-1)], axis=-1)

        # ee_forces:
        force_torque = self.robot.get_force_torque_sensor(self.robot.right_end_effector - 1)

        obs = [end_effector_pos_real, end_effector_orient_real,
    ...
    def reset(self, stiffness=[5., 1., 1.5], friction=0.5, cloth_scale=0.1, cloth_mass=0.5):
        ...
        # Put table for panda agent:
        if isinstance(self.robot, Panda):
            self.panda_stand = p.loadURDF(os.path.join(self.directory, 'nightstand', 'nightstand.urdf'),
                                          basePosition=np.array([0.9, 0., 0]), baseOrientation=[0, 0, 1, 1],
                                          physicsClientId=self.id)

            base_pos = [0.8, 0.1, 0.68]

        # joint_angles = [1.14317203, 0.18115511, 1.36771027, -0.77079951, 0.28547459, -0.6480673, -1.58786233]
        joint_angles = [3,  -.5,  -2.5, -1.5, -1.0,  2.0,  0.]
        ...
        # TODO: find the joint positions that should place the robot in the correct initial position

        pos_offset = np.asarray([0, -0.08, 0.05])
        wrist_pos = self.human.get_pos_orient(self.human.left_wrist)[0]
        end_effector_pos, end_effector_orient = self.robot.get_pos_orient(self.robot.left_end_effector)


        deseried_ee_pos = wrist_pos+pos_offset
        # deseried_ee_pos = end_effector_pos
        desired_ee_or = end_effector_orient
        desired_ee_or = self.get_quaternion([0, -np.pi*3/4, 0])
        # desired_ee_or = self.get_quaternion([0, 0, 0])


        success, joint_angles = self.robot.ik_random_restarts(right=False, target_pos=deseried_ee_pos, target_orient=desired_ee_or, max_iterations=1000, max_ik_random_restarts=40,
                               success_threshold=0.03, step_sim=False, check_env_collisions=False,
                               randomize_limits=True, collision_objects=[])
        ...
        self.cloth = p.loadSoftBody(os.path.join(self.directory, 'clothing', 'gown_696v.obj'),
                                    #os.path.join(self.directory, 'cloth3d/Tshirt', '0.obj'),
                                    scale=1.0,
                                    mass=0.15,
                                    useBendingSprings=1,
                                    useMassSpring=1,
        ...
        # self.cloth = p.loadSoftBody(os.path.join(self.directory, 'clothing', 'gown_696v.obj'), scale=1.0, mass=3, useNeoHookean=1,
        ...
        p.changeVisualShape(self.cloth, -1, rgbaColor=[0, 0, 1, 0.5], flags=0, physicsClientId=self.id)
