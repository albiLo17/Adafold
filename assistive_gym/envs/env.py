...
class AssistiveEnv(gym.Env):
    def __init__(self, robot=None, second_robot=None, human=None, task='', obs_robot_len=0, obs_human_len=0, time_step=0.02, frame_skip=10, render=False, gravity=-9.81, seed=1001, deformable=False, scale_factor=5):
    ...
    def reset(self):
        ...
        # ## camera for elas
        # p.resetDebugVisualizerCamera(cameraDistance=15.75, cameraYaw=-0.0, cameraPitch=-90.1,
        #                              cameraTargetPosition=[-0.2, 5, 0.4], physicsClientId=self.id)
        # ### camera for bend
        # p.resetDebugVisualizerCamera(cameraDistance=5.75, cameraYaw=-0.0, cameraPitch=-0.1,
        #                              cameraTargetPosition=[-0.0, 0.0, 10.0], physicsClientId=self.id)
        # ### camera for table
        # p.resetDebugVisualizerCamera(cameraDistance=1.75, cameraYaw=-45.0, cameraPitch=-0.1,
        #                              cameraTargetPosition=[-0.0, -0.5, 2.0], physicsClientId=self.id)

        # p.resetDebugVisualizerCamera(cameraDistance=0.5, cameraYaw=-45.0, cameraPitch=-40,
        #                              cameraTargetPosition=[0.3, -0.3, 1.0], physicsClientId=self.id)      # Previous folding visualization


        # GOOD CAMERA POSITION FOR HALF FOLDING
        # p.resetDebugVisualizerCamera(cameraDistance=0.5, cameraYaw=-45.0, cameraPitch=-40,
        #                              cameraTargetPosition=[0., -0., 0.1], physicsClientId=self.id)

        # GOOD CAMERA POSITION FOR DRESSING
        # (lateral)
        # p.resetDebugVisualizerCamera(cameraDistance=1.5, cameraYaw=45.0, cameraPitch=-40,
        #                              cameraTargetPosition=[0.2, -0.1, 1.0], physicsClientId=self.id)
        # (front)
        p.resetDebugVisualizerCamera(cameraDistance=1., cameraYaw=0.40, cameraPitch=-45,
                                     cameraTargetPosition=[0.2, -0.1, 1.0], physicsClientId=self.id)
        # (top)
        # p.resetDebugVisualizerCamera(cameraDistance=1.5, cameraYaw=45.0, cameraPitch=-40,
        #                              cameraTargetPosition=[0.2, -0.1, 1.0], physicsClientId=self.id)
    ...
    def build_assistive_env(self, furniture_type=None, fixed_human_base=True, human_impairment='random', gender='random', mass=None, body_shape=None):
        ...
        if self.robot is not None:
            ...
            self.robot.init(self.directory, self.id, self.np_random, scale_factor=self.scale_factor, fixed_base=not self.robot.mobile)
            self.agents.append(self.robot)
        if self.second_robot is not None:
            self.second_robot.init(self.directory, self.id, self.np_random, self.scale_factor, fixed_base=not self.second_robot.mobile)
    ...
    def get_camera_image_depth(self, light_pos=[0, -3, 1], shadow=True, ambient=0.8, diffuse=0.3, specular=0.1):
    ...
    def get_point_cloud(self, depth=None, rgb=None, light_pos=[0, -3, 1], shadow=False, ambient=0.8, diffuse=0.3, specular=0.1, object_id=None):

        # get a depth image
        # "infinite" depths will have a value close to 1
        image_arr = p.getCameraImage(self.camera_width, self.camera_height,
            self.view_matrix, self.projection_matrix,
            shadow=shadow,
            lightDirection=light_pos, lightAmbientCoeff=ambient, lightDiffuseCoeff=diffuse, lightSpecularCoeff=specular,
            renderer=p.ER_TINY_RENDERER,
    ...
    def create_joint_sphere(self, radius=0.01, mass=0.10, pos=[0, 0, 0], visual=True, collision=True, rgba=[0, 1, 1, 1], maximal_coordinates=True, return_collision_visual=False):
        sphere_collision = p.createCollisionShape(shapeType=p.GEOM_SPHERE, radius=radius, physicsClientId=self.id) if collision else -1
        sphere_visual = p.createVisualShape(shapeType=p.GEOM_SPHERE, radius=radius, rgbaColor=rgba, physicsClientId=self.id) if visual else -1
        if return_collision_visual:
            return sphere_collision, sphere_visual
        body = p.createMultiBody(baseMass=mass, baseCollisionShapeIndex=sphere_collision, baseVisualShapeIndex=sphere_visual, basePosition=pos, useMaximalCoordinates=maximal_coordinates, physicsClientId=self.id)
        # joint_id = p.createConstraint(body, -1, -1, -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0], [0, 0, 0])
        sphere = Agent()
        sphere.init(body, self.id, self.np_random, indices=-1)
        # return sphere, joint_id
        return sphere, body
