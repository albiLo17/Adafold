...
class ClothObjectEnv(AssistiveEnv):
    ...
    def _get_obs(self):
        ...
        mesh = p.getMeshData(self.cloth, -1, flags=p.MESH_DATA_SIMULATION_MESH, physicsClientId=self.id)

        return [mesh_points, first_force_torque_reading, second_force_torque_reading, first_linkstate[0], first_linkstate[1], second_linkstate[0], second_linkstate[0], mesh]
    ...
    def reset(
        ...
        base_pos = [robot_init_x*self.scale_factor, 0.40101663*self.scale_factor, 1*self.scale_factor]
        base_orient = [0, 0, 1, 0]
        ...
        if self.context_task == "task":
            ...
            sphere_pos = (robot_base_pos + second_robot_base_pos) / 2 + np.array([0.0*self.scale_factor, 0*self.scale_factor, 0.55*self.scale_factor])
